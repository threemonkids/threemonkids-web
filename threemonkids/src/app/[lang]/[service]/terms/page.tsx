...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!legal) return { title: dict.legal.terms_heading };

  const l = lang as Lang;
  const workTitle = (l === "ko" ? legal.title_ko : legal.title_en) ?? service;
  return { title: `${dict.legal.terms_heading} — ${workTitle}` };
}
...
export default async function TermsPage({ params }: Props) {
  ...
  const l = lang as Lang;
  const content = l === "ko" ? legal.terms_ko : legal.terms_en;

  return (
    <LegalPage
      lang={l}
      legal={legal}
      heading={dict.legal.terms_heading}
      content={content}
